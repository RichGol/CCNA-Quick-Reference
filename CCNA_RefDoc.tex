...
\usepackage{multirow}										% provide multi-row table cells
...
\usepackage{enumitem}									% provide itemize/enumerate formatting
...
\section{Layer 1 Standards \label{sec:L1}}
	...
	\begin{table}[H]
	\centering
	\caption{Power over Ethernet (PoE) Standards \label{tab:POE}}
	\begin{tabular}{| l l | r @{ } r r | l @{ } l |}\hline
	Cisco Inline Power	& ILP			& 		& 7.5W	& 48v		&				& Mode B\\\hline
	IEEE 802.3af			& Type 1 PoE	& class 0-3	& 15.4W	& 44-57v	& $\ge$ CAT3		& Mode A/B\\\hline
	IEEE 802.3at			& Type 2 PoE+	& class 0-4	& 30W	& 50-57v	& $\ge$ CAT5		& Mode A/B\\\hline
	\mr{2}{IEEE 802.3bt}	& Type 3 UPoE	& class 0-6	& 60W	& 50-57v	& \mr{2}{$\ge$ CAT5}	& \mr{2}{4Pair PoE}\\
					& Type 4 UPoE+	& class 0-8	& 100W	& 52-57v	&				& \\\hline
	\end{tabular}\end{table}
...
\section{Layer 2 Protocols \label{sec:L2}}
	...
	0000.5e00.01XX 		& \RFC{5798} VRRP\\\hline
	...
	Preamble		& 7		& Bytes		& 10101010 (\texttt{0xaa})\\\hline
	SFD 			& 1		& Byte		& 10101011 (\texttt{0xab})\\\hline
	Destination MAC	& 6		& Bytes		&\\\hline
	Source MAC	& 6		& Bytes		&\\\hline
	Type (Ethernet II)	& \mr{2}{2}	& \mr{2}{Bytes}	& EtherType (\Cref{tab:ETHERTYPE})\\
	Length (802.3)	&		&			& Payload Length in Bytes\\\hline
	Data			& \mc{2}{c|}{varies}		& 46-1500 Bytes based on MTU\\\hline
	FCS			& 4 & Bytes				&\\\hline
	\end{tabular}\end{table}
	...
	\begin{tabular}{| l | r @{ } l | r |}\hline
	Destination Address	& 40	& bits		& Multicast \texttt{0100.0c00.00} or \texttt{0300.0c00.00}\\\hline
	...
	\centering
	\begin{tabular}{| l | r @{ } l | r |}\hline
	Protocol Identifier		& 2		& Bytes		& STP / PVST+ (\texttt{0x0000})\\\hline
	\mr{2}{Version}		& \mr{2}{1}	& \mr{2}{Byte}	& STP / PVST+ (\texttt{0x00})\\
					&		&			& RSTP / PVRST (\texttt{0x02})\\\hline
	\mr{2}{Message Type}	& \mr{2}{1}	& \mr{2}{Byte}	& Identify Configuration / TCN BPDUs\\
					&		&			& RSTP/MSTP (\texttt{0x02})\\\hline
	...
	Origin HW 	& 48	& bits	& Source L2 Address\\\hline
	Origin IP	& 32	& bits	& Source L3 Address\\\hline
	Target HW 	& 48	& bits	& Destination L2 Address\\\hline
	Target IP	& 32	& bits	& Destination L3 Address\\\hline
	\end{tabular}\end{table}
...
%	0x3c 	& 60			& IPv6-Opts\\\hline
	...
	224.0.0.18		& \RFC{5798} VRRP		&\\\hline
	...
	\textbf{Address Class}	& \textbf{Block}			& \mc{4}{c}{\textbf{Address Format}}\\\hline
	Global			& \mr{2}{Any}			& \mc{2}{r!{+}}{Prefix}				& Subnet ID					& Interface ID\\
	Unicast			&					& \mc{2}{r!{\hphantom{+}}}{$P$ bits}	& \mc{1}{r!{\hphantom{+}}}{$64-P$ bits}	& 64 bits\\\hline
	Unique Local		& \mr{2}{\texttt{fd00::/8}}	& \texttt{fd}	& Global ID			& Subnet ID					& Interface ID\\
	Unicast			&					& \mc{1}{r!{\hphantom{+}}}{8 bits} & \mc{1}{r!{\hphantom{+}}}{40 bits} & \mc{1}{r!{\hphantom{+}}}{16 bits} & 64 bits\\\hline
	Link-Local			& \mr{2}{\texttt{fe80::/64}}	& \mc{3}{r!{+}}{\texttt{fe80:0:0:0}}									& Interface ID\\
	Unicast			&					& \mc{3}{r!{\hphantom{+}}}{64 bits}									& 64 bits\\\hline
	Multicast			& \texttt{ff00::/8}			& \mc{4}{r@{}}{\texttt{ff02::1:ffXX:XXXX/104} Solicited-Node}\\
					& \texttt{ff01::/16}		& \mc{4}{r@{}}{Interface/Node-Local}\\
					& \texttt{ff02::/16}		& \mc{4}{r@{}}{Link-Local}\\
					& \texttt{ff05::/16}		& \mc{4}{r@{}}{Site-Local}\\
					& \texttt{ff08::/16}		& \mc{4}{r@{}}{Org-Local}\\
					& \texttt{ff0e::/16}		& \mc{4}{r@{}}{Global}\\\hline
	\end{tabular}\end{table}
	...
	30.0	& Traceroute\\\hline
	42.0	& Extended Echo Request\\\hline
	43.X 	& Extended Echo Reply\\\hline
	\end{tabular}\end{minipage}\hfill
	...
	A \textit{Stub Area} is a non-backbone OSPF area with Type 3 Default and Type 3 Summary LSAs. A \textit{Totally Stubby Area} is a stub area with no Type 3 Summary LSAs. A \textit{Not-So-Stubby Area} (NSSA) is a stub area with Type 7 LSAs. A \textit{Totally NSSA} is a totally stubby area with Type 7 LSAs.
...
%\textit{Software-Defined Networking} (SDN) is a new approach to building and controlling an IP-based network using a centralized control plane and \textit{SDN Controller} (controller) software. This controller may be placed anywhere within the network. \textit{Southbound Interface(s)} (SBIs) enable communication with networking devices, while \textit{Northbound Interface(s)} (NBIs) enable the use of network management applications on the controller. An SDN model often supports multiple SBIs/NBIs; each interface is defined by a communication protocol (OpenFlow, NetConf, PCEP, SSH, Telnet) and an API (REST).
%
%SDN interfaces enable \textit{network programmability} (automated network control via application software) and network data analysis through the controller. \textit{Configuration Management} features interact with device configurations (version control). \textit{Operational Management} features monitor the network and alert to potential issues (configuration drift).
%
%The \textit{Open Networking Foundation} (ONF) consortium helps define protocols, SBIs/NBIs, and visions within the SDN marketplace. They created the \textit{OpenSDN} model and \textit{OpenFlow} SBI for a completely centralized control plane. An OpenSDN controller knows the network topology and individual device configurations, enabling it to directly program device tables. OpenSDN switches offer feature flexibility beyond that of traditional Layer 2 / 3 switches.
%
%Enterprise SDN consolidation during the 2010s resulted in the open-source \textit{OpenDaylight} (ODL) controller. Under the Linux Foundation, vendors are allowed to use the ODL controller as a basis for their own SDN controllers. Cisco used it to build their proprietary \textit{OpenSDN Controller} (OSC), which has since been discontinued.
%
%Cisco \textit{Intent-Based Networking} (IBN) approach to SDN offers \textit{Application Centric Infrastructure} (ACI) for data centers, \textit{Software-Defined Access} (SDA) for enterprise campus LANs, and \textit{Software-Defined WAN} (SD-WAN) for enterprise WANs. See \href{https://developer.cisco.com}{Cisco Devnet} and \href{https://dcloud.cisco.com}{Cisco dCloud} to learn more about Cisco IBN offerings.
...
\section{Appendix \label{sec:APPENDIX}}
	...
	%IEEE 802.1x / 802.11i Wireless Security 																	--tab:WIRELESS SECURITY
	\begin{table}[H]
	\centering
	\caption{IEEE 802.1x/IEEE 802.11i Extensible Authentication Protocol (EAP) \label{tab:WIRELESS SECURITY}}
	\begin{tabular}{| m{2.75cm} | m{2.75cm} | W{c}{6cm} | W{c}{4.5cm} |}\hline
	\multirow{8}{=}{Authentication}	& \mc{2}{c|}{Open Authentication (Open)}				& 802.11 compliance\\\cline{2-4}
		& \mc{2}{c|}{Wired Equivalent Privacy (WEP)}									& Static WEP Keys\\\cline{2-4}
		& \multirow{6}{=}{IEEE 802.1x / 802.11i EAP}	& Lightweight EAP (LEAP)				& Dynamic WEP Keys\\\cline{3-4}
			&& EAP Flexible Authentication by										& Protected Access\\
			&& Secure Tunneling (EAP-FAST)										& Credential (PAC)\\\cline{3-4}
			&& Protected EAP (PEAP)											& Digital Certificate (AS)\\\cline{3-4}
			&& EAP Transport Layer Security										& Digital Certificate\\
			&& (EAP-TLS)													& (AS + Client)\\\hline
	\multirow{6}{=}{Privacy and Integrity}	& \mc{2}{c|}{\mr{2}{Temporal Key Integrity Protocol (TKIP)}}	& WPA Personal\\
		& \mc{2}{c|}{}														& WPA Enterprise\\\cline{2-4}
		& \mc{2}{c|}{\mr{2}{AES Counter/CBC-MAC Protocol (CCMP)}}						& WPA2 Personal\\
		& \mc{2}{c|}{}														& WPA2 Enterprise\\\cline{2-4}
		& \mc{2}{c|}{\mr{2}{AES Galois/Counter Mode Protocol (CCMP)}}						& WPA3 Personal\\
		& \mc{2}{c|}{}														& WPA3 Enterprise\\\hline
	\end{tabular}\end{table}
%	\begin{figure}[H]
%	\centering
...
configure terminal
	...
	snmp-server {contact | location | chassis-id} TEXT
...
configure terminal
	...
	ip access-list resequence {ACL_ID | ACL_NAME} START STEP
...
configure terminal
	...
	tacacs server NAME
		...
		port PORT
		single-connection
	ip {radius | tacacs} source-interface INT
	aaa group server {radius | tacacs+} AAA_GROUP
		server {A.B.C.D [auth-port PORT] [acct-port PORT] | name NAME}
...
%	\textbf{IEEE 802.1x Configuration}
%	\begin{lstlisting}
%configure terminal
%	radius-server dead-criteria time SECS tries COUNT
%	radius-server deadtime MINS
%	radius-server vsa send {authentication | accounting}
%	radius-server attribute 6 on-for-login-auth
%	radius-server attribute 8 include-in-access-req
%	radius-server attribute 25 access-request include
%	ip device tracking
%	ip device tracking probe delay SECS
...
%		switchport {access | voice} vlan VLAN_ID
%		switchport mode access
%		authentication port-control {auto | force-authorized | force-unauthorized}				!-- enable dot1x closed mode
%		dot1x pae authenticator													!-- act as a dot1x authenticator
%		mab																!-- enable MAC Auth Bypass (MAB)
%		dot1x timeout quiet-period SECS 											!-- configure delay between auth attempts
%		authentication host-mode {[single-host | multi-host | multi-domain | multi-auth] [mab]}		!-- configure dot1x host mode and MAB
%		authentication {order | priority} {[dot1x] [mab]}									!-- configure the auth-type order and preemption
%
%		authentication periodic													!-- configure periodic re-auth
%		authentication timer reauthenticate {SECS | server}								!-- configure re-auth timer 
%
%		authentication {open | closed}												!-- configure dot1x monitor / closed mode
%		ip access-group PRE-AUTH in 												!-- configure dot1x low-impact mode (with dot1x monitor)
%		authentication event fail action next-method
%		authentication event server dead action authorize {vlan VLAN_ID | voice}					!-- configure critical data VLAN / enable critical voice VLAN
%		authentication event server alive action reinitialize								!-- configure reinitialization when RADIUS server recovers
%
%show authentication sessions [interface INT]
%	\end{lstlisting}
%
%	\textbf{Telephony Services Configuration}
%	\begin{lstlisting}
%configure terminal
%	telephony-service															!-- configure telephony services
%		max-ephones COUNT													!-- set max allowed VoIP Phones
%		max-dn COUNT														!-- set max allowed DNs
%		ip source-address A.B.C.D port PORT										!-- source traffic via ip A.B.C.D on TCP PORT
%		load MODEL FIRMWARE												!-- set MODEL phones to download FIRMWARE from flash: via tftp
%		create cnf-files														!-- re-create phone configuration files
%	ephone-dn DN															!-- configure an individual DN
%		number EXT														!-- set an extension EXT for the DN
%	ephone PHONE_ID														!-- configure a single VoIP Phone
%		mac-address MAC													!-- map the VoIP Phone MAC to a PHONE_ID
%		type MODEL														!-- specify the phone model number
%		button1:DN															!-- map the VoIP Phone and PHONE_ID to a DN
%show telephony-service														!-- view telephony-serivce configuration
%	\end{lstlisting}
...
show ip bgp
	...
	\textbf{Multicast Configuration}
	\begin{lstlisting}
configure terminal
	{ip | ipv6} multicast-routing
	{ip | ipv6} pim rp-address A.B.C.D
	ip pim send-rp-announce INT scope TTL
